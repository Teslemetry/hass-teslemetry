...
async def _async_get_energy_site_live_status(energy_site: EnergySite) -> Any:
    """Fetch live status for an energy site, raising auth errors properly."""
    try:
        return (await energy_site.live_status())["response"]
...
async def async_setup_entry(hass: HomeAssistant, entry: TeslemetryConfigEntry) -> bool:
    ...
    for product in products:
        if (
            ...
            live_status = await _async_get_energy_site_live_status(energy_site)
