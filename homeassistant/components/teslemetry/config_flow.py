    ...
    async def async_step_credentials(
        ...
        if user_input is not None:
            ...
            try:
                ...
                entry = self._get_entry()
                # async_update_reload_and_abort raises ValueError here: the
                # vehicle subentry flow's removal-reload listener
                # (_setup_subentry_removal_reload) is always registered on the
                # entry, and core refuses to combine that with a declarative
                # reload. Update and reload explicitly instead, same as the
                # vehicle pairing flow's _async_finish.
                result = self.async_update_and_abort(
                    entry,
                    self._get_reconfigure_subentry(),
