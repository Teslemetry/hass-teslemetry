    ...
    async def async_step_scan(
        ...
        if user_input is not None:
            ...
            if device is None:
                ...
                except (BleakError, TeslaFleetError, TimeoutError):  # fmt: skip
    ...
    async def async_step_credentials(
        ...
        if user_input is not None:
            ...
            try:
                ...
                # async_update_reload_and_abort refuses to run when the entry
                # has update listeners (the vehicle-BLE removal watcher always
                # registers one), so reload explicitly instead - same pattern
                # that watcher itself uses.
                self.hass.config_entries.async_schedule_reload(entry.entry_id)
                return result
