...
- **Multi-exception paren hardening** (`calendar.py`, and check `config_flow.py`) - Python 3.14's PEP 758 allows `except A, B:` without parens, and `dev`'s own source occasionally uses that form. It compiles fine on the dev box's 3.14 but is a `SyntaxError` on any older stable Python. After every PR apply, grep the tree for `except \w+, \w+` and parenthesize each hit as `except (A, B):  # fmt: skip` (the comment stops `ruff format` from stripping the parens back). Verify with a stable-version `python3.13 -m compileall`, not just the dev box's `python3.14` - 3.14 alone will not catch a regression here. Retire once the stable support floor reaches 3.14.
