...
- **`tpms_atm_gate`** — no dev-only `UnitOfPressure.ATM` in `sensor.py`; see "HACS-only patches" below.
...
- **`services_exceptions_gate`** — every `translation_key="..."` raised in `services.py` has a non-empty `exceptions.<key>.message` in both `strings.json` and the compiled `translations/en.json`. A core-dev sync can resolve `services.py` correctly (keep the shim) while `strings.json` has no textual conflict and silently takes upstream's deletion of the matching `exceptions` keys, orphaning the shim's error messages with a green build. Happened twice (release-6.0.19, release-6.0.20) before landing on `main`. Never hand-edit `translations/en.json` to satisfy it; fix `strings.json` and recompile. Retire alongside the `include_child_devices` drop.
...
- **`include_child_devices` drop** (`services.py`, `async_get_device_for_service_call`) — **TEMPORARY.** Core dev calls `device_registry.async_get(device_id, include_child_devices=False)`; the kwarg first ships in core 2026.9.0 and raises `TypeError` on every released core, so all device-targeted Actions fail with a generic "unknown error" while the dev-form build gate stays green. The fork drops the kwarg (`async_get(device_id)`), which is behaviourally identical because Teslemetry never parents devices (it sets only `via_device_id`, which core forbids from being a child), so a targeted device is always a main device — a `cast` keeps the narrow `DeviceEntry` return type. The core-`dev` sync **re-conflicts on this line every cut**: keep the fork's kwarg-less call, never accept upstream's. The shim's two `translation_key` exception messages (`invalid_device`, `no_config_entry_for_device`) must survive every sync too — `services_exceptions_gate` enforces it. Retire the drop, `services_child_devices_gate`, `services_exceptions_gate`, and this bullet together once the minimum core floor reaches 2026.9.0. The shim must also survive `DeviceEntry.config_entries` deprecation: read `config_entry_id` when present and fall back to `config_entries` only on cores that lack it.
...
- **Stable-core `UnitOfPressure.ATM` compat** (`sensor.py`) — **TEMPORARY.** `sensor.py`'s four streamed TPMS sensors MUST convert atm to bar via `_tpms_atm_to_bar` (atm → Pa by the literal 101325 ratio, then `PressureConverter.convert(..., UnitOfPressure.PA, UnitOfPressure.BAR)`) and MUST NEVER reference `UnitOfPressure.ATM` in code (a compat comment mention is allowed). That enum member is dev-only (first released 2026.10) and absent on the stable cores HACS users run, so referencing it raises `AttributeError` in `teslemetry_stream`'s `_dispatch` on every streamed tire-pressure update — the four TPMS sensors stop updating. The break is stable-only, so the dev-form build gate passes green; `tpms_atm_gate`'s grep is the only thing that catches it. The core-`dev` sync re-introduces the dev form on this line every cut — keep the fork's shim, never accept upstream's. Retire the shim, the gate, and this bullet together once the minimum core floor reaches 2026.10.
