...
- **Vehicle BLE key-storage grep** (`keystore_gate`) - asserts the composed vehicle BLE private key path resolves under `STORAGE_DIR`, never the bare config root. Guards the "Vehicle BLE key storage" standing patch below, which `apply_prs` can never apply (it rides a fork branch, not an open PR). Retires gracefully: inert when the composed tree carries no vehicle key handling, so it keeps protecting the invariant after #176296 merges and the patch step is removed, and never fires spuriously.
...
- **Vehicle BLE key storage** (`const.py`, `helpers.py`, `__init__.py`) - moves the vehicle BLE private key from the user-browsable config root to `hass.config.path(STORAGE_DIR, "teslemetry_vehicle_key.pem")`, **migrating** an existing config-root key rather than regenerating it (a fresh key strands any vehicle already paired against the old one). Unlike the patches above it **cannot live on `main`**: the code it edits only exists once core PR #176296's Bluetooth work is applied, so it rides on #176296's head as a fork branch (`fm/teslemetry-ble-key-storage` on `Bre77/core`), not an open PR - `apply_prs`, which enumerates Bre77's *open* PRs, never finds it, and it is applied by hand today. That is the shape that already lost the back-migration and the aiopowerwall pin, and it is the most fragile of the three. `release.sh` applies it mechanically in `apply_standing_patches` (the `STANDING_PATCHES` data table), **after** `apply_prs` because it is based on the post-#176296 tree; a patch that will not apply stops for a human exactly like a PR patch, never a silent skip, and an unresolvable remote/branch is a hard stop. `keystore_gate` fail-stops the cut if the composed vehicle key path is not under `STORAGE_DIR` - same green-build failure shape as the two above (both key locations import cleanly). Retire when #176296 merges upstream and the key-storage fix ships as the first follow-up core PR: it then arrives through the ordinary dev sync, and the patch step, the gate and this bullet come out together.
