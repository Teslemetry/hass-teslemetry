...
- Before `pytest`, translations must be compiled for the integration (`python3 -m script.translations develop --integration teslemetry`) or `tests/components/conftest.py`'s `check_translations` fixture fails every test that raises a repair/issue, even though the strings are correct.
