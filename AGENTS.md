...
- **Minimum core floor** (`hacs.json` `homeassistant`) - a HACS-side standing minimum, currently `2026.8.0`, and the single place the floor is stated. It is the real floor, not a nicety, and it is set by an enum **member**, not just an enum class: `climate.py`, `media_player.py` and `update.py` import `ClimateEntityStateAttribute`/`MediaPlayerEntityStateAttribute`/`UpdateEntityStateAttribute` at top level (those classes first ship in core `2026.7.0`), and `climate.py` reads `ClimateEntityStateAttribute.TARGET_TEMPERATURE`, a member that first ships in core `2026.8.0` (before it the enum carries only `TEMPERATURE`). So below `2026.7.0` the integration fails to import outright, and on `2026.7.0`-`2026.7.4` it imports but every climate entity raises `AttributeError` while restoring state - either way it cannot run. The build gate can't catch this: it runs against dev-form core, where the member exists. Every cut MUST keep `hacs.json` at or above the last shipped floor, and MUST floor on the newest `*EntityStateAttribute` member actually accessed - a name-only check of the enum class passes a too-low tag (this is why the floor sits a minor above the enum classes' own `2026.7.0` arrival). Retire/raise only in step with the members `climate.py`/`media_player.py`/`update.py` access.
