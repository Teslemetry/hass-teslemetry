...
- **Minimum core floor** (`hacs.json` `homeassistant`) - a HACS-side standing minimum, currently `2026.7.0`, and the single place the floor is stated. It is the real floor, not a nicety: `climate.py`, `media_player.py` and `update.py` import `ClimateEntityStateAttribute`/`MediaPlayerEntityStateAttribute`/`UpdateEntityStateAttribute` at top level, and `ClimateEntityStateAttribute` first ships in core `2026.7.0` - below it the integration fails to import outright, so a too-low floor lets users on older cores install a build that cannot load. It regressed once (v6.0.9 advertised `2025.6.0`; `v6.0.3` correctly had `2026.7.0`), and the build gate can't catch it (it runs against dev-form core, where the members exist). Every cut MUST keep `hacs.json` at or above the last shipped floor. Retire/raise only in step with the top-level `*EntityStateAttribute` imports that drive it.
