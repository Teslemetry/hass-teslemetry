...
- For OAuth2 integrations that register their own `AbstractOAuth2Implementation`/application credential (e.g. dynamic client registration) rather than requiring the user to supply one: starting a config flow for a domain does **not** guarantee that domain's own `async_setup()` has already run in the current session — `_async_get_flow_handler` only imports the `config_flow` module and sets up *dependencies*, not the domain itself. Integrations that need the credential to exist before `async_step_user` (e.g. because reauth/reconfigure funnel through `super().async_step_user()`, bypassing a subclass override) must perform the "ensure credential registered" step in both the component's `async_setup()` (covers existing entries and reauth, which always run after `async_setup`) and in the config flow's own `async_step_user` override (covers a brand-new install where `async_setup` may not have run yet).
