...
- **If a PR has multiple commits, `gh pr diff --patch` returns the raw per-commit series (mailbox format), not one net diff.** `git apply -3` then applies each commit's hunks in sequence, which can produce nested/garbled conflict markers on files a later commit in the same PR reverts or re-touches (check with `gh pr diff --name-only`, which IS the net diff, against what `git apply` reports touching). When that happens, abandon the patch replay for the affected files (`git checkout HEAD -- <file>`) and do a real three-way merge instead: fetch the PR's base/head commits (`gh pr view $PR_NUMBER --json baseRefOid,headRefOid`, `git fetch upstream <sha>`), then `git merge-file -L ours -L base -L head <working-copy> <(git show <base>:<path>) <(git show <head>:<path>)`. This gives clean single-level conflict markers instead of nested ones.
...
- **Opt-in ClickStack log shipping** - the `logship` acquire/release block in `async_setup_entry` plus `logship.py`. Shipping has a single durable authorization gate: a per-entry config option (`ship_logs_to_clickstack`, set via the options flow in `config_flow.py`, default off), tracked as a force-count on the per-`hass` `TeslemetryLogShipper` singleton. It is deliberately *not* tied to the live DEBUG log level - that coupling used to silently drop shipping on restart whenever the user's debug-logging choice wasn't "persistent". `TeslemetryLogShipper.is_shipping_authorized()` is the single source of truth. An options-flow update reloads the entry (`_setup_options_reload_on_change` in `__init__.py`) so the force-count re-derives correctly. That listener must stay scoped to the `ship_logs_to_clickstack` option specifically, not `entry.add_update_listener` unconditionally - `async_add_subentry`/`async_update_subentry` (vehicle BLE pairing, energy site pairing) also fire update listeners, and an unscoped listener double-reloads the entry on every setup with the real (unpatched) `PLATFORMS`, which among other things breaks any test that restricts platforms via `patch("homeassistant.components.teslemetry.PLATFORMS", ...)`.
- **Corollary for any future subentry-flow PR** (currently #176296 vehicle BLE pairing, #176328 energy site local pairing - both still open, so re-applied every cycle until merged upstream): a `ConfigSubentryFlow`'s `async_update_reload_and_abort` hard-raises `ValueError` if the entry has any update listener, which this fork's entry always does. Their subentry flows must use `async_update_and_abort` plus an explicit `self.hass.config_entries.async_schedule_reload(entry.entry_id)` instead (see `VehicleSubentryFlowHandler`/`EnergySiteSubentryFlowHandler` in `config_flow.py`) - re-check this each time either PR's diff changes its subentry-flow completion step.
