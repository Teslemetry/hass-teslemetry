...
- **`include_child_devices` drop** (`services.py`, `async_get_device_for_service_call`) - **TEMPORARY, same failure class as the device_tracker shim.** Core PR #178666 added `device_registry.async_get(device_id, include_child_devices=False)` to teslemetry's own service helper on dev; the kwarg is dev-core-only (first ships 2026.9.0) and raises `TypeError` on every released core, so all device-targeted Actions fail with a generic "unknown error" and the dev-form build gate stays green. The fork drops the kwarg (`async_get(device_id)`); this is behaviourally identical because Teslemetry never parents devices (it sets only `via_device_id`, which core forbids from being a child), so a targeted device is always a main device - a `cast` keeps the narrow `DeviceEntry` return type. Because the fork diverges from upstream on this line, the core-`dev` sync **re-conflicts here every cut**: resolve by keeping the fork's kwarg-less call, never accept upstream's. `release.sh`'s `services_child_devices_gate` fail-stops the release if the kwarg reappears in code. Retire the drop, the gate, and this bullet together once the minimum core floor reaches 2026.9.0.
