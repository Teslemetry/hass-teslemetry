...
def async_register_services(hass: HomeAssistant) -> None:  # noqa: C901
        ...
        schema=vol.Schema(
            {
                ...
                vol.Required(ATTR_LOCALE): cv.string,
                vol.Optional(ATTR_TIMESTAMP): cv.positive_int,
            }
        ),
    )
    ...
    async def stream_fields(call: ServiceCall) -> None:
        ...
        try:
            resp = await vehicle.stream.get_vehicle(vehicle.vin).update_config({"fields":call.data[ATTR_FIELDS]})
